...
(function(global, factory) {
     ...
     */
    var defaultConfig = {
        cdnDomain: 'mg.127.net/static/qiye-official',  // CDN 完整域名带路径前缀
        fallbackDomain: typeof location !== 'undefined' ? location.origin : '',  // 使用当前主域
        cdnAvailable: null,  // null=未知, true=可用, false=不可用
        testTimeout: 3000,   // CDN 测试超时时间（毫秒）
        testImagePath: '/new/img/logo.5d2411d5.png'  // CDN 测试图片路径
    };
    ...
    function getCdnUrlRegex() {
        ...
        var escapedDomain = config.cdnDomain.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
     ...
     */
    function generateFallbackUrl(cdnUrl) {
        // URL 转换规则: CDN域名/xxx -> 当前主域/xxx
        var regex = getCdnUrlRegex();
     ...
     */
    function markElementProcessed(element, type) {
        if (type === 'error') {
     ...
     * @returns {boolean}
     */
    function isElementProcessed(element, type) {
        if (type === 'error') {
     ...
     */
    function handleScriptFallback(script, fallbackUrl) {
     ...
     * @param {string} fallbackUrl - 降级 URL
     */
    function handleLinkFallback(link, fallbackUrl) {
     ...
     * @param {string} fallbackUrl - 降级 URL
     */
    function handleImageFallback(img, fallbackUrl) {
        // 处理 <picture> 中的 <source> 标签
        var pictureParent = img.parentElement;
        if (pictureParent && pictureParent.tagName === 'PICTURE') {
            var sources = pictureParent.querySelectorAll('source[srcset*="' + config.cdnDomain + '"]');
            sources.forEach(function(source) {
                ...
                if (sourceSrcset && isCdnUrl(sourceSrcset)) {
                    var fallbackSrcset = generateFallbackUrl(sourceSrcset);
    ...
    function handleResourceError(element) {
        var resourceUrl = element.src || element.href;
        ...
        // 生成降级 URL
        var fallbackUrl = generateFallbackUrl(resourceUrl);
     ...
     */
    function replaceInlineBackgroundImage(element) {
        ...
        var regex = getCdnUrlRegex();
        var newStyle = inlineStyle.replace(regex, config.fallbackDomain + '/');
     ...
     */
    function replaceComputedBackgroundImage(element, hasBgLazy) {
        ...
        // 提取所有 URL（可能有多张背景图）
        var urlMatches = computedStyle.match(/url\(["']?([^"')]+)["']?\)/g);
        if (!urlMatches) {
            return;
        }
        ...
        urlMatches.forEach(function(urlMatch) {
            // 提取单个 URL
            var url = urlMatch.match(/url\(["']?([^"')]+)["']?\)/);
            if (!url || !url[1] || !isCdnUrl(url[1])) {
                return;
            }
            
            var originalUrl = url[1];
            var fallbackUrl = generateFallbackUrl(originalUrl);
            ...
            if (hasBgLazy) {
                ...
                if (currentStyle && currentStyle !== 'none') {
                    // 替换 CDN URL（转义特殊字符）
                    var escapedUrl = originalUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                    var newBgImage = currentStyle.replace(
    ...
    function replaceBackgroundImage(element) {
        ...
        var hasBgLazy = element.classList.contains('bg-lazy');
        if (hasBgLazy) {
    ...
    function replaceAllSources() {
        var sources = document.querySelectorAll('source[srcset*="' + config.cdnDomain + '"]');
        sources.forEach(function(source) {
            if (isElementProcessed(source, 'error')) return;
    ...
    function replaceAllImages() {
        var images = document.querySelectorAll('img[src*="' + config.cdnDomain + '"]');
        images.forEach(function(img) {
            if (isElementProcessed(img, 'error')) return;
            
            if (!isCdnUrl(img.src)) return;
            
            var fallbackUrl = generateFallbackUrl(img.src);
    ...
    function replaceAllBackgroundImages() {
        ...
        var bgLazyElements = document.querySelectorAll('.bg-lazy');
     ...
     */
    function testCdnAvailability(callback) {
        // 如果已经测试过，直接返回结果
        ...
        // 使用测试图片进行检测
        var testUrl = getCdnBaseUrl() + config.testImagePath + '?' + Date.now();
        testImg.src = testUrl;
    }
    ...
    function handleNewElement(node) {
        ...
        if (node.tagName === 'SOURCE' && node.srcset && isCdnUrl(node.srcset)) {
            if (!isElementProcessed(node, 'error')) {
                var fallbackSrcset = generateFallbackUrl(node.srcset);
        ...
        if (node.querySelectorAll) {
            // 处理子元素中的图片
            var images = node.querySelectorAll('img[src*="' + config.cdnDomain + '"]');
            images.forEach(function(img) {
            ...
            // 标记子元素中的 bg-lazy
            var bgLazyElements = node.querySelectorAll('.bg-lazy');
            bgLazyElements.forEach(function(element) {
                element.dataset.cdnFallbackPending = 'true';
            });
            
            // 处理子元素的背景图（延迟处理，确保 CSS 样式已应用）
            var allChildElements = node.querySelectorAll('*');
    ...
    function init(options) {
        ...
        if (typeof document !== 'undefined') {
            ...
            function doPreCheck() {
                ...
                testCdnAvailability(function(isAvailable) {
                    if (!isAvailable) {
                        // CDN 不可用，执行批量替换
                        executeReplaceAll();
                    } else {
     ...
     */
    function checkElementBackground(element) {
        ...
        // 检查所有子元素
        var allChildElements = el.querySelectorAll('*');
    ...
    var publicAPI = {
         ...
         * @param {Element} element - 要处理的元素
         */
        replaceBackground: function(element) {
            if (!config) {
                console.warn('⚠️ 请先调用 init() 初始化');
                return;
            }
